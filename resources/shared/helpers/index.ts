...
export function toQueryString<T extends Record<string, unknown>>(obj: T, prefix = ''): string {
...
export function parseKeyParts(key: string): (string | number)[] {
  return key
    ...
    .map((part, index) => {
      ...
      return index > 0 && /^\d+$/.test(camelPart) ? parseInt(camelPart, 10) : camelPart;
    });
}

// Рекурсивное создание структуры объекта
export function assignDeep(target: Record<string, unknown>, keys: (string | number)[], value: unknown): void {
  let current = target;
...
export function prepareQueryParams<T extends Record<string, unknown>>(params?: T): Record<string, string> {
  if (!params) return {};
...
export function getFreeSpace(parentElement: HTMLElement) {
  ...
  Array.from(parentElement.children).forEach((child) => {
    ...
    const width = style.display === 'flex' ? child.scrollWidth : child.offsetWidth;

    const height = style.display === 'flex' ? child.scrollHeight : child.offsetHeight;
